...
### WEATHER CALIBRATED (2026-06-29) — mean is EFFICIENT; distribution untested

- Got exact settlement stations from market rules: NYC=Central Park, Chicago=MIDWAY (not O'Hare — my probe used O'Hare), Miami=Intl, LA=LAX, Denver=Intl. Settlement = NWS Daily Climatological Report; rules warn of C→F rounding nuances at bucket boundaries.
- Re-ran with correct stations + NWS official forecast + multi-model ensemble (gfs025+ecmwf+icon) vs the market's price-implied expected high:
  NYC mkt 88.1 / NWS 87 (-1.1); CHI 95.1 / 96 (+0.9); MIA 94.2 / 94 (-0.2); LA 70.3 / 67 (-3.3, but ensemble 71.9 sides with market); DEN 91.0 / 92 (+1.0).
- **The market's expected high matches NWS within ~1°F for 4/5 cities. The 72% "edge" was a single-model (gfs025 ran 3°F hot) ARTIFACT, now confirmed dead.** Market = NWS = efficient on the MEAN. LA gap is NWS being the outlier (hard coastal forecast), not a market mispricing.

...
- **Logo fix**: Deleted old `favicon.ico` from scaffolding that was overriding the correct SVG logo via metadata.icons in layout.tsx.
- **Resolution notification filter**: Only sends Slack/email for markets that had recommendations (not all resolved markets).
...
- **Daily scan at 8 AM PT**: Default scan times `[8]` (single daily scan). Configurable via Settings UI. Research shows sports lines are softest at 6-9 AM ET before sharp money arrives.
...
- **Economics bypass Haiku screening**: Economics markets skip Haiku pre-screening since `_detect_economics()` already curates them. Haiku was incorrectly rejecting all Fed rate markets as "obvious" (25 found, 0 researched). Sports still go through Haiku.
- **Extreme-price filter**: Scanner now skips markets with price <= 0.02 or >= 0.98 (was <= 0 or >= 1.0). Markets at 98%+ or 2%- are essentially certain with no edge potential — saves API calls.
...
- **Binary complement dedup**: `_deduplicate_event_markets()` in scanner.py groups markets by `event_ticker` and keeps only the highest-volume market per event. Prevents researching both "Will Tauson win?" and "Will Stearns win?" for the same match. Selection: highest volume, tiebreak by price closest to 0.50. Runs after date filtering, before AI research.
- **1x daily scan**: Default `scan_times` changed from `[5, 8, 14]` to `[8]` (single scan at 8 AM PT). Configurable via Settings UI.

**Scheduler:** APScheduler running (configurable scan times defaulting to 8 AM PT using batch mode, 1h resolution check, trade sync every 4h when enabled, daily digest 9 PM PT when notifications enabled, hourly stale rec cleanup, price checks disabled by default). Scan schedule is dynamically reconfigurable from Settings UI.

    ...
    async def screen(self, blind_input: BlindMarketInput) -> bool:
        ...
        if category == "economics":
            ...
            prompt = (
                ...
                f"- Game totals: 'Will total points be over/under X?' (team or combined)\n"
                f"- Margin of victory: 'Will Team X win by 5+ points?'\n"
                f"- Team performance props: 'Will Team X score 110+ points?'\n"
                f"- Selection/participation: 'Will Player X be selected for Event Y?'\n"
                f"- Star player scoring thresholds with clear statistical basis\n"
                ...
                f"BAD markets (say NO):\n"
                f"- Highly random props: first basket, coin toss, first foul\n"
                f"- Trivial ceremony outcomes: slam dunk contest, skills challenge\n"
                f"- Bench player stats (role players with unpredictable minutes)\n"
